...
reduce_lr = ReduceLROnPlateau(monitor='val_loss',  #observar a função de perda no conjunto de validação
                            ...
                            verbose=1, 
                            patience=100,    # número de épocas, sem redução na função de perda, que o monitor espera para agir.
...
train = [X_img_tr, X_num_tr]
val = [X_img_val, X_num_val]
test = [X_img_te, X_num_te]
...
history = model.fit(train, y_tr_cat, batch_size=32, epochs=5000, callbacks=[reduce_lr,early_stopping], verbose=1, validation_data=(val, y_val_cat))

# history = model.fit(train,y_tr_cat ,nb_epoch=89,                              validation_data=(val, y_val_cat),
#                               verbose=0,
...
loss, accuracy = model.evaluate(val, y_val_cat, verbose=0)
...
frame = pd.DataFrame(model.predict(test),index=ID, columns=classes)
frame.to_csv('submisiondf_combine.csv')
